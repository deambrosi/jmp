...
%
%   This is an enhanced version of simulateAgents that, in addition to
%   locations, wealth, states, and network status, also records:
%       - Period-by-period income
%       - Whether the latest arrival at each location was achieved with help
%
...
function income = computeIncomeFromState(locationIdx, stateIdx, params, grids, dims)
    etaIdx  = floor((stateIdx - 1) / dims.B) + 1;
    etaIdx  = max(1, min(etaIdx, numel(grids.eta)));
    etaVal  = grids.eta(etaIdx);
    income  = params.A(locationIdx) * etaVal;
end
