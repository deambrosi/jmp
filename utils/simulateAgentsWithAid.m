function [M_history, MIN_history, agentData, aid_count] = simulateAgentsWithAid(m0, pol, G_dist, dims, params, grids, settings, aidParams)
% SIMULATEAGENTS Simulates agent evolution over T periods using policy functions.
%
%   [M_history, MIN_history, agentData, aid_count] = simulateAgentsWithAid(...)
%
...
%       aidParams - Struct with aid settings:
%                     .t_start  : first period when aid may be given
%                     .a_under  : wealth index threshold for eligibility
%                     .p_aid    : probability of receiving aid
%                     .a_aid    : additional asset amount if aided
%                     .max_aid  : maximum total aid events
%
%   OUTPUTS:
...
%       aid_count   - Total number of aid payments granted
%
...
% =========================================================================
    ...
    aid_count    = 0;  % total aid disbursed
    ...
    for agentIdx = 1:numAgents
        ...
        for t = 1:(T-1)
            ...
            % Potential aid before migration choice
            nextWeaOrig = nextWea;
            aidGiven    = false;
            if t >= aidParams.t_start && loc == 1 && wea <= aidParams.a_under && ...
                    aid_count < aidParams.max_aid
                if rand() < aidParams.p_aid
                    aidGiven = true;
                    aid_count = aid_count + 1;
                    newA = grids.agrid(nextWea) + aidParams.a_aid;
            ...
            % Remove temporary aid if agent stays in location 1
            if aidGiven && nextLoc == 1
                nextWea = nextWeaOrig;
            end
